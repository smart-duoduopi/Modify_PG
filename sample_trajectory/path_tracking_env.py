...
class VehicleDynamics(object):
    ...
    def compute_rewards(self, obs, actions):  # obses and actions are tensors
        with tf.name_scope('compute_reward') as scope:
            ...
            rewards = 0.4 * devi_y + 0.1 * devi_phi + 0.2 * punish_yaw_rate + 5 * punish_steer
        return rewards
...
class ReferencePath(object):
    ...
    def compute_x_point(self):
        x = np.linspace(0., 100., 1000).astype(np.float32)
        return x
...
class PathTrackingModel(object):  # all tensors
    ...
    def rollout_out(self, actions):  # obses and actions are tensors, think of actions are in range [-1, 1]
        ...
        self.obses[:, 0:4] = self.vehicle_dynamics.prediction(self.obses[:, 0:4], self.actions)
